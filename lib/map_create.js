...
function initMap() {
    ...
    for (let i = 0; i < station_coords.length-1; i++) {
        ...
        if (currentStationName in allStations) {
            if (allStations[currentStationName].includes(currentStationCoords)) {
                continue;
            } else {
    ...
    for (const station in allStations) {
        allStations[station].forEach(stationName => {
            addStation({ lat: stationName.LATITUDE, lng: stationName.LONGITUDE }); 
        })
    }
